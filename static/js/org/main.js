...
const SERVER_ADDR = 'http://127.0.0.1:8889'
...
async function run() {
  ...
  if (!isFaceDetectionModelLoaded()) {
    await getCurrentFaceDetectionNet().load('../static/data/weights')
  }
}
    ...
    if (xhr.readyState === 4) {
      ...
      if (xhr.status == 200) {
        ...
        if (res.status == 'Enroll OK') {
          ...
          document.getElementById('result_text1').innerText = 'You already'
          document.getElementById('result_text1').style.color = 'red'
